...
void handleJsonData(String payload) {
  ...
  String type = doc["type"]; // Lấy giá trị của loại led
  int station  = doc["station"];         // Lấy giá trị của id trạm 
  String data = doc["data"];        // Lấy giá trị của data
  String gene = doc["gene"]; 
  int price = doc["price"]; 
  String combinedString = String(gene) + ":" + String(price);
  ...
  if (type != NULL && type == "info") {
      Serial.print("data: ");
      Serial.println(data);
      sendDataToSlave(ADDRESS_SLAVE1, data);
  } else if (type != NULL && type == "price") {
    Serial.print("data: ");
    Serial.println(combinedString);
    sendDataToSlave(ADDRESS_SLAVE2, combinedString);
  } else{}
