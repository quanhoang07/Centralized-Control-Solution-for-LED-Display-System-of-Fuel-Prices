...
void loop() {
  ...
  if (packetSize) {
    ...
    while (LoRa.available()) {
      ...
      if (senderAddress == ADDRESS_SLAVE1) {
        ...
        isScrolling = true;
        sendFeedback("Slave 1 da nhan duoc");
      } else {
        sendFeedback("Slave 1 khong nhan duoc");
      }
  ...
  scrollText(currentMessage, myRED);
}

// send feedback
void sendFeedback(String data1) {
  for (int i=0;i<5;){
      if ( (unsigned long) (millis() - time2) > 2000 ) {
          Serial.print("Sending packet: ");
          Serial.println(counter);
          sendResponseToMaster(data1);
          counter++;
          time2 = millis();
          i++;
        }
...
// send respond
void sendResponseToMaster(String data2) {
    LoRa.beginPacket();
    LoRa.write(ADDRESS_MASTER); // Ghi địa chỉ của master
    LoRa.write(ADDRESS_SLAVE1); // Ghi địa chỉ của người gửi (slave)
    LoRa.print(data2); // Ghi phản hồi vào gói
    LoRa.endPacket();
}
// hien thi ra led
void display(String data) {
  ...
  dma_display->print(data); // In văn bản
}

// cuon text
void scrollText(String text, uint16_t color) {
  dma_display->setTextWrap(false); // Không tự xuống dòng
  int16_t x = dma_display->width();
  int16_t y = (dma_display->height() - 8) / 2; // Tâm giữa màn hình
  
  // Tính chiều rộng của chuỗi dựa trên kích thước chữ
  uint8_t textSize = 2; // Kích thước chữ hiện tại (setTextSize)
  uint16_t charWidth = 6 * textSize; // Chiều rộng mỗi ký tự (6 pixel là kích thước mặc định của font)
  uint16_t textWidth = text.length() * charWidth;

  while (x + textWidth > 0) {
    dma_display->fillScreen(myBLACK); // Xóa màn hình
    dma_display->setCursor(x, y);    // Đặt vị trí bắt đầu in
    dma_display->setTextColor(color);
    dma_display->print(text);        // In chuỗi
    dma_display->flipDMABuffer();    // Cập nhật LED
    x -= 1;                          // Dịch sang trái
    delay(20);                       // Điều chỉnh tốc độ cuộn
  }
}
