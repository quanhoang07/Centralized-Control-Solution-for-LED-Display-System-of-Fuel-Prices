...
void updateDisplay(String currentMessage ) {
    if (isScrolling && currentMessage != "") { // Chỉ cuộn nếu có tin nhắn
        unsigned long currentTime = millis();
        if (currentTime - lastScrollTime > 50) { // Kiểm soát tốc độ cuộn
            lastScrollTime = currentTime;

            // Xóa màn hình và hiển thị tin nhắn
            ...
            // Kiểm tra nếu văn bản đã cuộn hết
            if (scrollX + currentMessage.length() * 6 < 0) {
                scrollX = dma_display->width(); // Đặt lại vị trí cuộn
            }
        }
    }
}
